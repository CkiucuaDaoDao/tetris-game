...
const canvas2 = document.getElementById('mini-board');
...
class Board {
  ...
  handleGameOver() {
    this.gameOver = true;
    this.isPlaying = false;
    confirm("Do you want to play agian ?");
  }
}
...
class MiniBoard {
  constructor(ctx2) {
    this.ctx2= ctx2;
    this.grid2 = this.generateWhiteMiniBoard()
  }
  
  generateWhiteMiniBoard() {
    return Array.from({ length: rows }, () => Array(cols).fill(WHITE_COLOR_ID))
  }

  drawMiniCell(xAxis, yAxis, colorId) {
    this.ctx2.fillStyle = COLOR_MAPPING[colorId] || COLOR_MAPPING[WHITE_COLOR_ID];
    this.ctx2.fillRect(
      xAxis * BLOCK_SIZE,
      yAxis * BLOCK_SIZE,
      BLOCK_SIZE,
  ...
  drawMiniBoard() {
    for( let row = 0; row < this.grid2.length; row++) {
      for ( let col = 0; col < this.grid2[0].length; col++) {
        this.drawMiniCell(col, row, this.grid2[row][col])
      }
    }
  }
}
...
class Brick {
  ...
  drawNextBrick() {
    for ( let row = 0; row < this.layout[this.activeIndex].length; row++) {
      for ( let col = 0; col < this.layout[this.activeIndex][0].length; col++) {
        if (this.layout[this.activeIndex][row][col] !== WHITE_COLOR_ID) {
          miniBoard.drawMiniCell(col + this.colPos, row + this.rowPos, this.id)
        }
      }
    }
  }
...
miniBoard = new MiniBoard(ctx2)
miniBoard.drawMiniBoard();
